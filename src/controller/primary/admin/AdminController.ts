 ...
 * @param res
 */
export async function cheakNumber(req: Request, res: Response): Promise<Response<any, Record<string, any>>> {
    try {
        const partnerRegistrationService = new PARTNER_SERVICE.PartnerService();
        const response = await partnerRegistrationService.checkNumber(req.body.phone_number);
        const result = _.map(response[0], (obj) => {
            return _.omit(obj, ['created_by', 'updated_by', 'updated_on', 'delete_status', 'password']);
        });
        if (result) {
